...
extrap_effect <- readRDS("extrap_effect.rds")
SEV_met <- readRDS("SEV_met.rds")
...
\title{Demographic back-casting reveals that subtle dimensions of climate change have strong effects on population viability}
...
\item
We found clear patterns of climate change in our central New Mexico study region but, contrary to our hypothesis, \textit{C. imbricata} has most likely benefitted from recent climate change and is on track to reach replacement-level population growth within \Sexpr{round(ms_quantities$viable_year_1970,0)-2020} years, or sooner if climate change accelerates. 
...
\item
\textit{Synthesis}.
Our results highlight the challenges of \revise{back-casting or} forecasting population dynamics under climate change, since the most apparent features of climate change may not be the most important drivers of ecological responses.
...
We hypothesized that high-sensitivity vital rates (those that strongly influence $\lambda$) would be less responsive environmental variability than low-sensivity vital rates \citep{pfister1998patterns}.
...
Finally, we asked whether the components of climate that are changing most strongly in this system are the same climate components that most strongly influence cactus demography. 
}
...
\revise{\linelabel{mod7}By relying on downscaled, interpolated climate data instead of direct observations from meteorological stations we are trading off local resolution in favor of more historical years of data.
...
We found that the two data sets were \revise{generally well correlated} (Table \ref{tab:WNASEV}, Fig. \ref{fig:WNASEV_temp},\ref{fig:WNASEV_precip}), which bolstered our confidence in ClimateWNA for back-casting demographic responses to climate over the historical record.
We further explored the implications of using downscaled data by repeating all of our analyses (described next) with SEV-LTER meteorological data and comparing results between the two data sources (Appendix A).
}
...
Finally, the linear predictor includes normally distributed random effects for plot-to-plot variation ($\phi \sim N(0,\sigma_{plot})$) and year-to-year variation that is unrelated to climate effects captured by PCs 1-3 ($\tau \sim N(0,\sigma_{year})$).
...
\noindent Functions $P$ and $F$ give the probability of flowering and the number of flowerbuds produced, respectively, for an $x$-sized plant. 
...
For analysis, we discretized $x$ into $b$ bins, replacing the continuous kernel with an $b$-by-$b$ matrix (because our model also included two additional discrete states, the final projection matrix had dimensions $b+2$-by-$b+2$).
We used $b=200$ bins. 
...
For the years of demographic data collection (2004--2017), \revise{\linelabel{mod2}we additionally quantified} the deviations between predicted $\lambda$ based solely on climate and ``observed'' $\lambda$ that reflects climate and non-climate year effects (quotations indicate that these are the asymptotic predictions given the vital rates observed in that year). 
We also conducted a similar analysis of $\lambda_{S}$ using a $10$-year sliding window (Appendix C), \revise{\linelabel{mod4}and we explored the consequences of extrapolating vital rate responses to climate values more extreme than those observed during the study period (Appendix D).}
...
\lambda_{t} = \bar{\lambda} + \sum_{i=1}^{3} \nu_{i}PC_{i,t}
\end{align}
...
\nu_{i} \approx \sum_{j=1}^{n} \frac{\partial \bar{\lambda}}{\partial \theta_{j}} \frac{\partial \theta_{j}}{\partial PC_{i}} 
\end{align}
The LTRE approximation is based on the product of the sensitivity of $\lambda$ to the vital rates ($\theta$), evaluated at the long-term mean climate ($PC_{1}=PC_{2}=PC_{3}=0$), and the sensitivity of the vital rates to climate, summed over all vital rates. 
...
Over the last century, PC1 trends have fluctuated, with prolonged stretches of warm and dry years (the 1950s and early 2000s) and other periods of cool and wet years (early 1900s and 1970s-80s), though the overall temporal trend for PC1 was negative.
The decline per-year was nearly five times stronger since 1970 compared to the long-term average (Fig. \ref{figure:PCA}C), suggesting an accelerating trajectory of warmer and drier years. 
...
PC2 has increased since 1900 and the change per-year was nearly four times stronger since 1970 (Figure \ref{figure:PCA}D), indicating an accelerating trend of wetter cool seasons with moderate winter temperatures. 
...
Thus, even with relatively strong climate-demography associations (Fig. \ref{figure:vital_rates}), there was substantial uncertainty in our back-casted estimates of $\lambda$.
\revise{\linelabel{mod5}The shaded region in Fig. \ref{figure:lambda_year}A represents the combined uncertainty arising from heterogeneity in vital rates across years that could not be attributed to the climate PCs (process error) and imperfect knowledge of the underlying parameters (estimation error).
In Appendix Fig. \ref{fig:lambda_year_proc_est_err}, we show that process error contributed the majority of the total uncertainty.}
...
Under the trajectory since 1970, population growth was expected to reach the viability threshold ($\lambda$ = 1) in the year \Sexpr{round(ms_quantities$viable_year_1970,0)} (Fig. \ref{figure:lambda_year}C); accelerating climate change would advance this transition to viable growth rates. 

\revise{\linelabel{mod3}In Appendix D, we show that our inference that $\lambda$ is likely increasing in response to climate change holds even with a more conservative approach that does not extrapolate vital rate responses beyond the climate extremes of the observation years.}
\revise{\linelabel{mod8}Furthermore, in Appendix A, we show that year-specific estimates of $\lambda$ were correlated between models built with downscaled climate data versus on-site meteorological measurements, for years in which they over-lapped (Fig. \ref{fig:lambda_t_comp}, \reviseagain{Fig. \ref{fig:SEV_WNA_time_series}}).
This suggests that our qualitative inference regarding the positive temporal trend in $\lambda$ is robust to the loss of resolution associated with downscaled climate data.}

The stochastic population growth rate ($\lambda_{S}$) showed a similar trend of $\lambda_{S} < 1$ and increasing population growth rates over the past $120$ years (Fig. \ref{fig:lambdaS}).
...
Our results \reviseagain{fail to support this hypothesis and} suggest the opposite: \textit{C. imbricata} is likely a climate change ``winner'', on an accelerating trajectory toward replacement-level population growth within \Sexpr{round(ms_quantities$viable_year_1970,0)-2020} years if current climate change trends persist, and sooner if they accelerate. 
...
Even with strong climate-demography relationships detected with our unusually long-term  data set, climate drivers accounted for \revise{less than two-thirds} of the inter-annual variation in $\lambda$ during the study years. 
...
We have emphasized the positive trajectory of population viability as the most likely trend in $\lambda$, but this should be interpreted in light of the probability distributions that we provide (Fig. \ref{figure:lambda_year}) -- that is, with nuance and appropriate caution\footnote{\revise{By coincidence, the probability that $\lambda$ is increasing ($\Sexpr{ms_quantities$positive_lambda_prob}$) matched the probability of a Clinton victory in the 2016 U.S. presidential election: \url{https://projects.fivethirtyeight.com/2016-election-forecast/}}}. 
...
\revise{\linelabel{mod9}Defining the temporal or spatial auto-correlation structure of process error (which we did not attempt) may further improve forecasts or back-casts.}
...
Indeed, previous work showed that \textit{C. imbricata} at lower \revise{\linelabel{mod10}(by ca. 100 m)} elevations had positive population growth rates \citep{miller2009impacts} and may therefore act as source populations. 
Regardless of which process or processes best account for the persistence of a population that is currently inviable, our results indicate that it will \linelabel{mod11}\revise{more likely than not} be `rescued' by ongoing climate change. 
...
This may be an issue of applied concern in the region since \textit{C. imbricata} is considered undesirable \linelabel{mod12}\revise{due to its unpalatability to livestock} \citep{allen1991diffusion}. 
...
Similarly, Salguero-Gomez et al. (\citeyear{salguero2012demographic}) identified \textit{Cryptantha flava} as \linelabel{mod13}\revise{a species likely to benefit from climate change} due in part to seasonal redistribution of rainfall that will lengthen its growing season. 

\revise{\linelabel{mod15} Our work highlights several considerations that may be relevant for studies of demographic back-casting in other systems. 
First, we faced a trade-off between temporal depth and local resolution of climate data.
While downscaled climate interpolation (from ClimateWNA) and on-site measurements (from SEV-LTER) were correlated, they were not perfectly so (Appendix A); this was especially true for temperature minima and maxima (Table \ref{tab:WNASEV}), where downscaled data likely mis-estimate localized extremes. 
We prioritized the greater temporal coverage provided by downscaled data, which led an \Sexpr{SEV_met$climate_rsq - ms_quantities$climate_rsq}\% reduction in how well climate explained inter-annual variation in $\lambda$, relative to on-site climate data (Appendix A). 
Consequently, reliance on downscaled data inflated the contribution of process error to our back-casted estimates (Appendix D), \linelabel{mod16} and made $\lambda$ appear less responsive to climate than it likely was. 
\linelabel{mod20}It is particularly noteworthy that the downscaled climate data poorly captured the deep-freeze of winter 2011  (Fig. \ref{fig:WNASEV_temp}A).
...
As expected, the on-site data predicted a lower $\lambda$ value in this year than the downscaled data (Fig. \ref{fig:lambda_t_comp}).
When available, climate data sources that break the trade-off between temporal depth and local resolution would provide the strongest foundation for accurate back-casting. 
When such resources are not available, quantifying the loss of resolution, as we have done (Appendix A), may be valuable for interpreting results.
}

\revise{\linelabel{mod17} Second, just like forecasting, demographic back-casting may require projection into climatic conditions that were represented poorly or not at all during the data collection period. 
\linelabel{mod18}This requires the assumption that the relationship between vital rates and climate covariates does not change or break down under conditions more extreme than observed. 
We found similar results whether or not we extrapolated demographic performance into unobserved conditions (Appendix D).
This was a lucky break, reflecting the fact that the climate covariate requiring the most extrapolation (PC1) had the weakest effect on $\lambda$. 
In other cases, where important covariates must be extrapolated to no-analogue conditions, comparing results with and without extrapolation (Appendix D) may be valuable for setting liberal and conservative bounds on model projections.
This approach may also aid in identifying situations where experimental climate manipulations could help bridge the gap between current and historic (or future) conditions. 
}
...
Some additional limitations of our study warrant consideration in the interpretation of our results. 
First, our treatment of climate dependence was limited to four vital rate processes of established plants.
...
However, consistent with expectations for long-lived perennials, we know seedling recruitment to have very low eigenvalue sensitivities \citep{elderd2016quantifying}, which suggests that even large climate effects on this process may not strongly register in terms of population growth.
\revise{\linelabel{mod19}On the other hand, pulsed recruitment events perturb the size distribution in ways that can importantly affect short-term (transient) dynamics \citep{williams2011distance}, and may therefore warrant further study in this and other pulsed-recruitment system. }
...
To conclude, this study illustrates how long-term patterns of population growth can be reconstructed \reviseagain{\linelabel{rev2-4}(with potentially substantial but quantifiable uncertainty)} through climate-demography relationships observed on relatively short time scales.
This allowed us to evaluate the hypothesis that recent climate change has driven \textit{C. imbricata} in our region into extinction debt, \linelabel{mod14}\revise{a hypothesis that our data do not support}.
Instead, this species is \revise{most likely benefitting from climate change}, largely due to its positive responses, especially in survival, to recent and ongoing shifts in cool-season temperature and precipitation. 
Changes in cool-season climate were not the strongest features of climate change, but they were nonetheless the most important determinants of population responses. 
...
This study was supported by the Sevilleta LTER (NSF LTER awards 1440478, 1655499, and 1748133) and by NSF Division of Environmental Biology awards 1543651 and 1754468.  
We thank the Sevilleta National Wildlife Refuge staff (especially J. Erz) for facilitating research access. 
We thank M. Evans and E. Schultz for helpful discussions on modeling climate-demography relationships.
Finally, we thank the many students and colleagues have contributed to this long-term study, especially M. Donald, A. Compagnoni, and B. Ochocki. 
Two reviewers provided helpful feedback on our work. 
...
@
\subsection*{Correlation of climate values}
...
There was moderate to strong agreement between the two data sources (Table \ref{tab:WNASEV}, Fig. \ref{fig:WNASEV_temp}, Fig. \ref{fig:WNASEV_precip}). 
Temperature extrema were less strongly correlated between the two data sets than temperature means (Fig. \ref{fig:WNASEV_temp}), which is unsurprising given that extreme values may be sensitive to local micro-environmental conditions that the relatively coarse downscaled data would miss. 
There was an extreme-cold event in 2011 that was particularly poorly captured by the downscaled data (Fig. \ref{fig:WNASEV_temp}A).
The weakest correlation was that of warm-season maximum temperature (Fig. \ref{fig:WNASEV_temp}F; Pearson's $r = \Sexpr{round(warm_max$estimate,2)}$, $P = \Sexpr{round(warm_max$p.value,2)}$). 
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/SEV-WNA_corr_temp.pdf}
  \caption{\revise{Correlations of minimum, mean, and maximum temperature values of cool (\textbf{A--C}) and warm (\textbf{D--F}) seasons between SEV-LTER metereological data and downscaled estimates from ClimateWNA for years 2004--2017. Diagonal lines show $y=x$.}}
   \label{fig:WNASEV_temp}
   \end{center}
\end{figure}

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/SEV-WNA_corr_precip.pdf}
  \caption{\revise{Correlations of cool- (\textbf{A}) and warm-season (\textbf{B}) precipitation between SEV-LTER metereological data and downscaled estimates from ClimateWNA for years 2004--2017. Diagonal lines show $y=x$.}}
   \label{fig:WNASEV_precip}
   \end{center}
\end{figure}
...
\revise{
\subsection*{Re-analysis with SEV-LTER data}
To further explore the consequences of relying on down-scaled climate data, we re-ran our demographic analysis using the SEV-LTER meteorological data and compared the results to those based on ClimateWNA.
First, we conducted PCA on raw seasonal temperature and precipitation values from SEV Meteorological Station 50 over the observation years 2004--2017. 
As in our analysis of ClimateWNA data, parallel analysis supported retention of three principal components. 
Variable loadings onto these PCs are shown in Fig. \ref{fig:SEV_PCA} and show a pattern similar to ClimateWNA data whereby PC1 swas dominated by annual differences (cool- and warm-season variables loaded similarly) and PC2-3 were dominated by seasonal climate factors. 
\reviseagain{\linelabel{rev2-1}However, seasonal variable loadings onto PC2 and PC3 were different for the two data sets (compare Figs. \ref{figure:PCA} and \ref{fig:SEV_PCA}).}
Second, we fit the full set of vital rate models to these three PCs and used stochastic variable selection (Appendix B) to eliminate weakly supported climate covariates.
When then re-fit the vital rate models including variables with $\hat{z_i} > 0.1$ (see Appendix B).
These fitted models are shown in Fig. \ref{fig:vital_rates_SEV}. 
\reviseagain{\linelabel{rev2-2}Note that the PC axes from SEV meteorological data are different PCs than those from ClimateWNA and have different variable loadings.
Thus, we expect differences in demographic responses between Figures \ref{figure:vital_rates} and \ref{fig:vital_rates_SEV}.}

We compared results based on the two data sources in several ways. 
First, we compared the inter-annual variances associated with year random effects in the statistical models. 
We found that, for survival in particular, random variance across years was much lower using SEV-LTER data as climate covariates compared to ClimateWNA (Fig. \ref{fig:sigma_year}).
This tell us that, as expected, on-site data provided greater resolution of climate drivers, since less inter-annual variation in survival was attributed to process error.

\reviseagain{\linelabel{rev2-3}Second, we used the IPM derived from each data source to generate two predicted time series of climate-sensitive vital rates (survival, flowering, and fertility) and $\lambda$ during the study years.
These time series are shown in Fig. \ref{fig:SEV_WNA_time_series}.
Year-specific estimates of flowering and fertility showed poor correspondence between the two data sources (Fig. \ref{fig:SEV_WNA_time_series}B,C), likely because they were both predicted to be more responsive to climate in the ClimateWNA analysis (Fig. \ref{figure:vital_rates}) compared to the SEV-LTER analysis (Fig. \ref{fig:vital_rates_SEV}).
However, year-specific survival rates were highly consistent between the two data sources (Fig. \ref{fig:SEV_WNA_time_series}A).
Because $\lambda$ was much more sensitive to survival than reproduction, year-specific estimates of $\lambda$ were also highly consistent and significantly correlated between the two data sources (Fig. \ref{fig:SEV_WNA_time_series}D, Fig. \ref{fig:lambda_t_comp}A; Pearson's $r=\Sexpr{round(SEV_met$lambda_corr$estimate,2)}$, $t_{\Sexpr{SEV_met$lambda_corr$parameter}}=\Sexpr{round(SEV_met$lambda_corr$statistic,2)}$, $P < \Sexpr{round(SEV_met$lambda_corr$p.value,2)}$). 
When we additionally incorporated year-specific random effects estimated from the statistical models, $\lambda$ estimates were nearly perfectly correlated (Fig. \ref{fig:lambda_t_comp}B; Pearson's $r=0.99$, $t_{\Sexpr{SEV_met$lambda_corr_RFX$parameter}}=\Sexpr{round(SEV_met$lambda_corr_RFX$statistic,2)}$, $P < 0.0001$). 
This tight correlation is expected, because year-specific random effects allow both the SEV-LTER and ClimateWNA models to match the observations, so it would be a sign of trouble if the relationship in Fig. \ref{fig:lambda_t_comp}B was weak. 
Finally, we found that SEV-LTER climate PCs explained \Sexpr{SEV_met$climate_rsq}\% of inter-annual variation in $\lambda$, an improvement over the \Sexpr{ms_quantities$climate_rsq}\% explained by ClimateWNA PCs. }

\reviseagain{
Overall, our re-analysis with SEV-LTER data and comparison between the on-site SEV and downscaled ClimateWNA data indicates that our qualitative conclusions about demography-climate relationships are robust to the choice of data source. 
In both analyses, we find vital rate responses to climate that translate to similar year-specific predictions for population growth rates. 
However, in relying on downscaled data for our main analyses, we certainly lost some of the climate signal.
The \Sexpr{SEV_met$climate_rsq - ms_quantities$climate_rsq}\% loss of resolution with ClimateWNA tells us that using downscaled data inflated the contribution of process error to our back-casted estimates (Fig. \ref{fig:lambda_year_proc_est_err}A).} 


\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.75\linewidth]{Figures/SevLTER_climate_PCA_loadings.pdf}
  \caption{\revise{Principal components analysis of SEV-LTER meteorological data. Bars show loadings of raw variables onto three principal components. Layout as in Fig. \ref{figure:PCA}.}}
   \label{fig:SEV_PCA}
   \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/vital_rates_SevLTER.pdf}
  \caption{\revise{Vital rate data and fitted models using principal components of SEV-LTER meteorological data as climate covariates. Layout as in Fig. \ref{figure:vital_rates}.}}
   \label{fig:vital_rates_SEV}
   \end{center}
\end{figure}

\newpage
<<SVS_table_SEV,echo=F, message=F, warning=F, results = 'asis'>>=
setwd("C:/Users/tm9/Desktop/git local/cholla_climate_IPM")
SVS_post_SEV<-read.csv("allrates_SVS_out_SevLTER.csv")[,c("X","mean")]
fert_z_SEV <- SVS_post_SEV %>% 
  filter(str_detect(X, 'fert.z')) %>% 
  slice(c(10,1:9)) %>% 
  select(mean) %>% 
  mutate(mean = round(mean,2)) %>% 
  mutate(mean = ifelse(mean>=0.1,paste0("$\\mathbf{", mean, "}$"),mean))
flow_z_SEV <- SVS_post_SEV %>% 
  filter(str_detect(X, 'flow.z')) %>% 
  slice(c(10,1:9)) %>% 
  select(mean) %>% 
  mutate(mean = round(mean,2)) %>% 
  mutate(mean = ifelse(mean>=0.1,paste0("$\\mathbf{", mean, "}$"),mean))
grow_z_SEV <- SVS_post_SEV %>% 
  filter(str_detect(X, 'grow.z')) %>% 
  slice(c(10,1:9)) %>% 
  select(mean) %>% 
  mutate(mean = round(mean,2)) %>% 
  mutate(mean = ifelse(mean>=0.1,paste0("$\\mathbf{", mean, "}$"),mean))
surv_z_SEV <- SVS_post_SEV %>% 
...
all_z_SEV <- bind_cols(PC_names,coef_names,surv_z_SEV,grow_z_SEV,flow_z_SEV,fert_z_SEV)
colnames(all_z_SEV) <- c("Climate PC","Model term","Survival","Growth","Flowering","Fertility")

print(xtable(all_z_SEV, caption = "Stochastic variable selection results based on climate data from SEV-LTER. Values (z) can be interpreted as the probability that a model coefficient is non-zero. Bolded values indicate terms retained in the final model.", label = "tab:SVS"), 
      include.rownames=F, 
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/sigma_year.pdf}
  \caption{\revise{Posterior distributions of inter-annual variance ($\sigma_{year}$) associated with year random effects from vital rate models fit with two climate data sources (colors): ClimateWNA and SEV-LTER. 
  Points show posterior means and bars show 95\% credible intervals.}}
   \label{fig:sigma_year}
   \end{center}
\end{figure}

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/SEV_WNA_time_series}
  \caption{\reviseagain{Year-specific estimates of vital rates (\textbf{A--C}) and population growth rates (\textbf{D}) based on SEV-LTER (filled points, solid lines) or ClimateWNA (open points, dashed lines). 
  Climate-dependent vital rates are probability of survival (\textbf{A}), probability of flowering (\textbf{B}), and flowerbud production of flowering plants (\textbf{C}).
  For each vital rate, colors correspond to three size groups: the 5th (gray), 50th (red), and 95th (blue) percentiles of the size distribution.
  SEV meteorological data were not available for 2003, so we could not estimate reproductive rates or population growth rates for the 2004 transition year.}}
   \label{fig:SEV_WNA_time_series}
   \end{center}
\end{figure}

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/lambda_t_comparison.pdf}
  \caption{\revise{Comparison of year-specific estimates of $\lambda$ from IPMs using either SEV-LTER ($x$-axis) or ClimateWNA ($y$-axis) as climate data sources.
  Diagonal lines show $y=x$.
  \textbf{A}, $\lambda$ estimates based only on climate PCs (Pearson's $r=\Sexpr{round(SEV_met$lambda_corr$estimate,2)}$, $t_{\Sexpr{SEV_met$lambda_corr$parameter}}=\Sexpr{round(SEV_met$lambda_corr$statistic,2)}$, $P < \Sexpr{round(SEV_met$lambda_corr$p.value,2)}$); \textbf{B}, $\lambda$ estimates based on climate and year random effects, which account for inter-annual differences not explained by the climate PCs (Pearson's $r=0.99$, $t_{\Sexpr{SEV_met$lambda_corr_RFX$parameter}}=\Sexpr{round(SEV_met$lambda_corr_RFX$statistic,2)}$, $P < 0.0001$).}}
   \label{fig:lambda_t_comp}
   \end{center}
\end{figure}
...
\begin{table}[h!]
  ...
  \begin{center}
    \begin{tabular}{p{5cm}p{2cm}p{1cm}p{5cm}}
      ...
      Survival year variance & $\sigma_{year}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="surv.sigma.year")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="surv.sigma.year")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="surv.sigma.year")],2)})\\
      Survival plot variance & $\sigma_{plot}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="surv.sigma.plot")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="surv.sigma.plot")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="surv.sigma.plot")],2)})\\
      \hline
      ...
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="grow.bsize")],2)})\\
       Growth residual variance & $\sigma$ & 
      ...
      Growth year variance& $\sigma_{year}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="grow.sigma.year")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="grow.sigma.year")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="grow.sigma.year")],2)})\\
      Growth plot variance& $\sigma_{year}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="grow.sigma.plot")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="grow.sigma.plot")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="grow.sigma.plot")],2)})\\
      \hline
      ...
      Flowering year variance& $\sigma_{year}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="flow.sigma.year")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="flow.sigma.year")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="flow.sigma.year")],2)})\\
      Flowering plot variance& $\sigma_{year}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="flow.sigma.plot")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="flow.sigma.plot")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="flow.sigma.plot")],2)})\\
      \hline
      ...
      Fertility year variance& $\sigma_{year}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="fert.sigma.year")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="fert.sigma.year")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="fert.sigma.year")],2)})\\
      Fertility plot variance& $\sigma_{year}$ & 
       \Sexpr{round(params_summ$mean[which(params_summ$X=="fert.sigma.plot")],2)} & 
      (\Sexpr{round(params_summ$X2.5.[which(params_summ$X=="fert.sigma.plot")],2)} --
      \Sexpr{round(params_summ$X97.5.[which(params_summ$X=="fert.sigma.plot")],2)})\\
      \hline
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/lambda_year_proc_est_err}
  \caption{\revise{\textbf{A}, Time series of back-casted asymptotic population growth rates ($\lambda$) predicted based on inter-annual variation in three climate PCs. 
Shaded regions show the 95\% credible interval of the posterior probability distributions for three uncertainty scenarios: estimation error only (parameter uncertainty; red), process error only (year-to-year heterogeneity unrelated to the climate PCs; blue), and both estimation and process error (black).
\textbf{B, C}, Posterior probability distribution for the change in $\lambda$ per year based on the entire time series (\textbf{B}) or years since 1970 (\textbf{C}). 
Vertical lines show the medians of the posterior distributions.
Colors as in \textbf{A}.}}
  \label{fig:lambda_year_proc_est_err}
  \end{center}
\end{figure}
...
\newpage
\revise{
\section*{Appendix D: Exploring the consequences of climate extrapolation}
...
Our analysis in the main text relied on extrapolating demographic responses to climate into climate environments that were not directly observed during our field study.
For example, high values of PC1 and low values of PC2 were under-represented during the study years (Fig. \ref{fig:climate_coverage_histograms}).  
We explored the consequences of this extrapolation by re-running our demographic analysis with bounds on climate responses.
For each vital rate that responded to a climate PC according to some function $f(PC)$, we defined a second function $f^*(PC)$ as:

%\[ 
\begin{equation}
    f^*(PC)= 
\begin{cases} 
    f(PC_{L}),& \text{if } PC < PC_{L}\\
    f(PC_{U}),& \text{if } PC > PC_{U}\\
    f(PC),  & \text{otherwise}
\end{cases} \label{eq:extrap}
\end{equation}
%\]

\noindent where $PC_{L}$ and $PC_{U}$ are the lower and upper bounds, respectively, of the observed range of PC values. 
For simulations into historical climates more extreme than observed, this approach pins demographic responses to equal the responses at observed extrema, as can be seen in $\lambda$ responses to PC variation (Fig. \ref{fig:lambda_PC_extrapF}). 
We repeated our back-casting analysis using this approach. 

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/climate_coverage_histograms}
  \caption{\revise{Distributions of observed climate values during the observation period (2004--2017) relative to historical values (1901-2016).
  Climate values are three principal components of inter-annual variation in cool- and warm-season temperature and precipitation.}}
  \label{fig:climate_coverage_histograms}
  \end{center}
\end{figure}

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/lambda_PC_extrapF}
  \caption{\revise{Relationships between $\lambda$ and three climate PCs with no extrapolation into unobserved climate conditions. 
  For PC values lower than the minimum (red vertical lines) and greater than the maximum (blue vertical lines) of the observation period, demographic responses were forced to match the extrema of the observation period according to Eq. \ref{eq:extrap}.}}
  \label{fig:lambda_PC_extrapF}
  \end{center}
\end{figure}

Results show that our qualitative results are not affected by climate extrapolation. 
The back-casted time series of $\lambda$ was generally consistent with and without extrapolation (Fig. \ref{fig:backcast_extrapTF}).
The main differences were in the extreme low $\lambda$ values, which were lower with extrapolation. 
Both time series yielded a positive temporal trend, though the mean change in $\lambda$ per year was \Sexpr{extrap_effect$extrap_effect}\% weaker for the entire time series and \Sexpr{extrap_effect$extrap_effect_1970}\% weaker since 1970 when vital rates were not extrapolated (Fig. \ref{fig:lambda_PC_extrapF}).
The limited influence of extrapolation was due to the fact that we relied most heavily on extrapolation for PC1 (Fig. \ref{fig:climate_coverage_histograms}). 
As we show in the main paper, this PC has changed the most during the historical record but it had the weakest effects on cactus demography. 

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/backcast_extrapTF}
  \caption{\revise{Back-casted values of climate-dependent population growth ($\lambda$) with (solid lines) and without (dashed lines) extrapolation of vital rate responses to unobserved climate conditions based on posterior mean parameter values.
  Red and blue lines show fitted regressions for the entire time series and since 1970, respectively.}}
  \label{fig:backcast_extrapTF}
  \end{center}
\end{figure}
}
\end{document}