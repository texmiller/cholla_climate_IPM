...
\revise{\linelabel{mod8}Furthermore, in Appendix A, we show that year-specific estimates of $\lambda$ were correlated between models built with downscaled climate data versus on-site meteorological measurements, for years in which they over-lapped (Fig. \ref{fig:lambda_t_comp}).
...
Our results suggest the opposite: \textit{C. imbricata} is likely a climate change ``winner'', on an accelerating trajectory toward replacement-level population growth within \Sexpr{round(ms_quantities$viable_year_1970,0)-2020} years if current climate change trends persist, and sooner if they accelerate. 
...
To conclude, this study illustrates how long-term patterns of population growth can be reconstructed through climate-demography relationships observed on relatively short time scales.
...
Second, we used the IPM derived from each data source to generate two predicted time series of $\lambda$ during the study years.
We found that SEV-LTER climate PCs explained \Sexpr{SEV_met$climate_rsq}\% of inter-annual variation in $\lambda$, an improvement over the \Sexpr{ms_quantities$climate_rsq}\% explained by ClimateWNA PCs. 
Finally, the year-by-year estimates were significantly correlated between the two data sources (Fig. \ref{fig:lambda_t_comp}A; Pearson's $r=\Sexpr{round(SEV_met$lambda_corr$estimate,2)}$, $t_{\Sexpr{SEV_met$lambda_corr$parameter}}=\Sexpr{round(SEV_met$lambda_corr$statistic,2)}$, $P < \Sexpr{round(SEV_met$lambda_corr$p.value,2)}$). 
When we additionally incorporated year-specific random effects estimated from the statistical models, $\lambda$ estimates were nearly perfectly correlated (Fig. \ref{fig:lambda_t_comp}B; Pearson's $r=0.99$, $t_{\Sexpr{SEV_met$lambda_corr_RFX$parameter}}=\Sexpr{round(SEV_met$lambda_corr_RFX$statistic,2)}$, $P < 0.0001$), as expected. 
This tells us that our qualitative conclusions based on the longer ClimateWNA time series of climate covariates is likely robust to the uncertainties introduced by downscaled data. 
However, the \Sexpr{SEV_met$climate_rsq - ms_quantities$climate_rsq}\% loss of resolution with ClimateWNA tells us that using downscaled data inflated the contribution of process error to our back-casted estimates (Fig. \ref{fig:lambda_year_proc_est_err}A). 
