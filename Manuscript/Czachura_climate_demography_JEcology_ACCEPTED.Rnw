...
\item
\textit{Synthesis}.
Our results highlight the challenges of back-casting or forecasting population dynamics under climate change, since the most apparent features of climate change may not be the most important drivers of ecological responses.
...
We found that the two data sets were generally well correlated (Table \ref{tab:WNASEV}, Fig. \ref{fig:WNASEV_temp},\ref{fig:WNASEV_precip}), which bolstered our confidence in ClimateWNA for back-casting demographic responses to climate over the historical record.
...
For the years of demographic data collection (2004--2017), we additionally quantified the deviations between predicted $\lambda$ based solely on climate and ``observed'' $\lambda$ that reflects climate and non-climate year effects (quotations indicate that these are the asymptotic predictions given the vital rates observed in that year). 
We also conducted a similar analysis of $\lambda_{S}$ using a $10$-year sliding window (Appendix C) and we explored the consequences of extrapolating vital rate responses to climate values more extreme than those observed during the study period (Appendix D).
...
The shaded region in Fig. \ref{figure:lambda_year}A represents the combined uncertainty arising from heterogeneity in vital rates across years that could not be attributed to the climate PCs (process error) and imperfect knowledge of the underlying parameters (estimation error).
...
In Appendix D, we show that our inference that $\lambda$ is likely increasing in response to climate change holds even with a more conservative approach that does not extrapolate vital rate responses beyond the climate extremes of the observation years.
Furthermore, in Appendix A, we show that year-specific estimates of $\lambda$ were correlated between models built with downscaled climate data versus on-site meteorological measurements, for years in which they over-lapped (Fig. \ref{fig:lambda_t_comp}, Fig. \ref{fig:SEV_WNA_time_series}).
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/lambda_year_uncertainty.pdf}
  \caption{
...
Our results fail to support this hypothesis and suggest the opposite: \textit{C. imbricata} is likely a climate change ``winner'', on an accelerating trajectory toward replacement-level population growth within \Sexpr{round(ms_quantities$viable_year_1970,0)-2020} years if current climate change trends persist, and sooner if they accelerate. 
...
Even with strong climate-demography relationships detected with our unusually long-term  data set, climate drivers accounted for less than two-thirds of the inter-annual variation in $\lambda$ during the study years. 
...
Defining the temporal or spatial auto-correlation structure of process error (which we did not attempt) may further improve forecasts or back-casts.
...
Indeed, previous work showed that \textit{C. imbricata} at lower (by ca. 100 m) elevations had positive population growth rates \citep{miller2009impacts} and may therefore act as source populations. 
Regardless of which process or processes best account for the persistence of a population that is currently inviable, our results indicate that it will more likely than not be `rescued' by ongoing climate change. 
...
This may be an issue of applied concern in the region since \textit{C. imbricata} is considered undesirable due to its unpalatability to livestock \citep{allen1991diffusion}. 
...
Similarly, Salguero-Gomez et al. (\citeyear{salguero2012demographic}) identified \textit{Cryptantha flava} as a species likely to benefit from climate change due in part to seasonal redistribution of rainfall that will lengthen its growing season. 

Our work highlights several considerations that may be relevant for studies of demographic back-casting in other systems. 
...
Consequently, reliance on downscaled data inflated the contribution of process error to our back-casted estimates (Appendix D), and made $\lambda$ appear less responsive to climate than it likely was. 
It is particularly noteworthy that the downscaled climate data poorly captured the deep-freeze of winter 2011  (Fig. \ref{fig:WNASEV_temp}A).
...
Second, just like forecasting, demographic back-casting may require projection into climatic conditions that were represented poorly or not at all during the data collection period. 
This requires the assumption that the relationship between vital rates and climate covariates does not change or break down under conditions more extreme than observed. 
...
On the other hand, pulsed recruitment events perturb the size distribution in ways that can importantly affect short-term (transient) dynamics \citep{williams2011distance}, and may therefore warrant further study in this and other pulsed-recruitment system. 
...
To conclude, this study illustrates how long-term patterns of population growth can be reconstructed (with potentially substantial but quantifiable uncertainty) through climate-demography relationships observed on relatively short time scales.
This allowed us to evaluate the hypothesis that recent climate change has driven \textit{C. imbricata} in our region into extinction debt, a hypothesis that our data do not support.
Instead, this species is most likely benefitting from climate change, largely due to its positive responses, especially in survival, to recent and ongoing shifts in cool-season temperature and precipitation. 
...
Our long-term data on tree cholla demography are publicly available \citep{cholla_dat_edi}. Code for our statistical and demographic modeling is available at \url{https://github.com/texmiller/cholla_climate_IPM} \citep{tom_miller_2020_3924640}.
...
\begin{figure}[H]
  ...
  \caption{Correlations of minimum, mean, and maximum temperature values of cool (\textbf{A--C}) and warm (\textbf{D--F}) seasons between SEV-LTER metereological data and downscaled estimates from ClimateWNA for years 2004--2017. Diagonal lines show $y=x$.}
...
\begin{figure}[H]
  ...
  \caption{Correlations of cool- (\textbf{A}) and warm-season (\textbf{B}) precipitation between SEV-LTER metereological data and downscaled estimates from ClimateWNA for years 2004--2017. Diagonal lines show $y=x$.}
...
However, seasonal variable loadings onto PC2 and PC3 were different for the two data sets (compare Figs. \ref{figure:PCA} and \ref{fig:SEV_PCA}).
...
Note that the PC axes from SEV meteorological data are different PCs than those from ClimateWNA and have different variable loadings.
...
Second, we used the IPM derived from each data source to generate two predicted time series of climate-sensitive vital rates (survival, flowering, and fertility) and $\lambda$ during the study years.
...
\begin{figure}[H]
  ...
  \caption{Principal components analysis of SEV-LTER meteorological data. Bars show loadings of raw variables onto three principal components. Layout as in Fig. \ref{figure:PCA}.}
   \label{fig:SEV_PCA}
   \end{center}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Vital rate data and fitted models using principal components of SEV-LTER meteorological data as climate covariates. Layout as in Fig. \ref{figure:vital_rates}.}
...
\begin{figure}[H]
  ...
  \caption{Posterior distributions of inter-annual variance ($\sigma_{year}$) associated with year random effects from vital rate models fit with two climate data sources (colors): ClimateWNA and SEV-LTER. 
...
\begin{figure}[H]
  ...
  \caption{Year-specific estimates of vital rates (\textbf{A--C}) and population growth rates (\textbf{D}) based on SEV-LTER (filled points, solid lines) or ClimateWNA (open points, dashed lines). 
...
\begin{figure}[H]
  ...
  \caption{Comparison of year-specific estimates of $\lambda$ from IPMs using either SEV-LTER ($x$-axis) or ClimateWNA ($y$-axis) as climate data sources.
...
\begin{figure}[h!]
  ...
  \caption{\textbf{A}, Time series of back-casted asymptotic population growth rates ($\lambda$) predicted based on inter-annual variation in three climate PCs. 
...
\begin{figure}[h!]
  ...
  \caption{Distributions of observed climate values during the observation period (2004--2017) relative to historical values (1901-2016).
...
\begin{figure}[h!]
  ...
  \caption{Relationships between $\lambda$ and three climate PCs with no extrapolation into unobserved climate conditions. 
...
\begin{figure}[h!]
  ...
  \caption{Back-casted values of climate-dependent population growth ($\lambda$) with (solid lines) and without (dashed lines) extrapolation of vital rate responses to unobserved climate conditions based on posterior mean parameter values.
