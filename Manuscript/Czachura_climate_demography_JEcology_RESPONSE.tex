...
\response{We thank the AE for their constructive comments on our paper.}
...
We now present the partitioning of process and estimation error in the new Figure C3, and describe these results qualitatively in the main text (l. \ref{ms-mod5}).
We found that process error contributed the majority of uncertainty in our back-casted estimates of $\lambda$ and we return to this result in the Discussion section (l. \ref{ms-mod16}).}
...
We have edited this text (l. \ref{ms-mod2}).}
...
\response{We see what the reviewer sees in Figure 4, but we interpret this a little differently. 
The bands of uncertainty make $\lambda$ appear relatively invariant, but we now show in Appendix D that much of this uncertainty comes from process error -- this means that the ``real'' values of $\lambda$ were likely bouncing around quite a bit. 
Furthermore, we also now show that our reliance on downscaled climate data likely inflated process error in this back-cast, because it made demography seem less sensitive to climate than it actually was (Appendix A, Fig. A6). 
Taken together, this suggests that the \textit{points} in Fig. 4 are the better representation of year-to-year fluctations. 
The uncertainty intervals simply provide bounds on those fluctuations. 
We now address this in the Discussion (l. \ref{ms-mod16}).}
...
\response{The sites differ in elevation by only ca. 100 meters and therefore do not strongly differ in climate. 
...
\response{This is a good point and is more naturally related to the issue of extrapolation, so we address it in that section of the Discussion (l. \ref{ms-mod18}).}
...
\response{Interesting point. We have included this idea in the Discussion (l. \ref{ms-mod19}).}
...
\response{Yes, we have done these new analyses (see response to comments by Reviewer 1). 
2010 and 2011 were the `worst' years according to both on-site and downscaled data, but the on-site estimates were lower, suggesting that they do a better job of capturing the freeze effect, as we now highlight in the Discussion (l. \ref{ms-mod20}). 
While the freeze occurred in Feb 2011 (during the 2010 transition year), it also affected estimates for the 2011 transition year (because some plants took a long time to die), as in seen in Fig. A7-A. }
