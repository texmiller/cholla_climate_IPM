...
Our study was designed to reconstruct how historical climate patterns affected population viability in \textit{C. imbricata} and to test the hypothesis that recent climate change has driven this population into extinction debt. 
...
\item{Back-cast climate-dependent demography to determine whether the past century included periods that were favorable for population growth}
...
We fit generalized linear mixed effects models in a hierarchical Bayesian framework to quantify climate dependence in demographic vital rates, as captured by three PCs that collectively explained \Sexpr{ms_quantities$var_explained}\% of the inter-annual variation in seasonal climate values (Results). 
...
\subsection*{Predicted and observed size distributions}
Predicted and observed size distributions generally corresponded well, though very large plants were over-represented in the observed distribution (Fig. \ref{figure:SSD}).
...
The ``new'' plants that we detected in our plots were likely several years old.
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/SSD.pdf}
