...
We found good qualitative correspondence between predicted and observed size distributions (), which bolstered our confidence that the model accurately described the dynamics of our focal population. 
...
Overall, recent climate change in each of the principal components, in isolation, has been in the direction that favors increased population growth (Fig. \ref{figure:lambda_PC}). 
...
For the the observation years (2004-2017) we were also able to incorporate additional sources of variability, estimated as statistical random effects, in year-specific estimates of $\lambda$ (points in Fig. \ref{figure:lambda_year}A).
For these years, the three climate PCs explained \Sexpr{ms_quantities$climate_rsq}\% of the inter-annual variation in $\lambda$.
...
To the contrary, there was a positive temporal trend ($\frac{\Delta\lambda}{\Delta Year} > 0$), suggesting a trajectory of increasing population growth rates through time (Figure \ref{figure:lambda_year}B).
There was wide uncertainty in the rate of change (corresponding to our imperfect ability to back-cast $\lambda$ based on climate alone) but the posterior probability distribution indicated that it was \Sexpr{round(ms_quantities$lambda_timesgreater,2)} times more likely that $\lambda$ has increased than decreased. 
...
There was also greater uncertainty in $\frac{\Delta\lambda}{\Delta Year}$ since 1970 since this estimate was based on fewer years.
Under the trajectory since 1970, population growth is expected to reach the threshold of positive population growth ($\lambda$ = 1) in \Sexpr{round(ms_quantities$viable_year_1970,0)}; accelerating climate change would likely advance this transition to increasing growth rates. 
...
Collectively, responses to PC2 and PC3 accounted for, on average, \Sexpr{ms_quantities$noPC1_rsq}\% of temporal variation in the back-casted values of $\lambda$. 
...
These generally corresponded well (Fig. \ref{figure:SSD}), though very large plants were over-represented in the observed size distribution.
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.5\linewidth]{Figures/SSD.pdf}
  \caption{NEED TO WRITE}
  \label{figure:SSD}
  \end{center}
\end{figure}
