...
Over the study period, average density of \textit{C. imbricata} was QUANTITY.
...
We used a fixed-design Life Table Response Experiment to decompose the total response  of $\lambda$ to climatic variation into contributions from each of the climate PCs and the underlying vital rate responses. 
...
All of the code for our statistical and demographic modeling is available at \url{https://github.com/texmiller/cholla_climate_IPM} and raw data is available through the Sevilleta LTER (\url{https://sevlter.unm.edu/data}).
...
Three principal components cumulatively explained \Sexpr{var_explained}\% of the inter-annual variation in climate (Figure \ref{figure:PCA}A); we focus on these three PCs as potential climate drivers of demography. 
...
Responses to PC3 varied in sign, with survival increasing with decreasing PC values (years with moderate winter temperature minima and wet summers) and reproductive rates increasing with increasing PC values (years with low winter minima and dry summers) (Fig. \ref{figure:vital_rates}C,I,L). 
...
The stochastic population growth rate ($\lambda_{S}$), calculated over a sliding $10$-year window, showed a similar trend of increasing population growth rates over the past $120$ (Appendix Figure).
...
Surprisingly, our results indicate the opposite: \textit{C. imbricata} is a climate change ``winner'', on an accelerating trajectory of increasing (less negative) population growth rates toward replacement-level within NN years if current climate change trends persist, and sooner if they accelerate. 
...
Flowering and fertility were strongly sensitive to climate but the asymptotic population growth rate was very weakly sensitive to these reproductive processes, such that they made virtually no contribution to the overall population response to climate (Fig. \ref{figure:vital_rates}, \ref{figure:lambda_year}).
...
In our study, we detected an important role for winter minimum temperature and observed the greatest annual mortality (percentage of all mortality events observed) in 2011, a year with record low winter temperatures over a multi-day deep-freeze (this is the low outlier in Fig. \ref{lambda_year}). 
...
Because we could not reliably assign new recruits a birth year, we did not incorporate climate dependence in seedling recruitment. 
Previous studies of cactus demography suggest that seedling recruitment may be highly sensitive to climate, especially monsoon precipitation (e.g., \citealt{bowers2005influence,winkler2018interaction}), and we suspect this is the case for \textit{C. imbricata}, since germination usually coincides with late-summer monsoons (\textit{T.E.X. Miller, unpubl. data}). 
Because we do not consider this process, our results for climate effects on population growth are conservative. 
...
A second limitation to consider is that our approach to quantifying climate drivers knowingly forfeits some information in two ways. 
First, we relied on downscaled climate projections rather than direct climatalogical observations.
While we know these two types of data to be highly correlated (Fig. \ref{fig:WNASEV}), they are not perfectly so; this is especially true for temperature minimia and maxima (Table \ref{tab:WNASEV}), where downscaled data likely miss localized extremes. 
It is noteworthy that the downscaled climate data poorly captured the extreme deep-freeze of 2011  (Fig. \ref{fig:WNASEV}); poor demographic performance in this year was consequently attributed to a statistical random effect (Fig. \ref{figure:lambda_year}A), though this was almost certainly a true climate effect. 
Second, we limit our consideration of climate drivers to the first three principal components of inter-annual variation.
While these three components explained a large majority of the variation (Fig. \ref{figure:PCA}A), we know that we are missing some of the more subtle dimensions of climate variability and change. 
...
These two factors mean that our conclusions for climate-dependence likely err on the conservative side.

Third, this was obviously an observational study, and we rely on PCA loadings to interpret mechanisms for responses to PC variation. 
There would be value in climate manipulation experiments to test these patterns. 
Bring up extrapolation.

Fourth, climate effects may include disruption of biotic interactions.

Our climate results are consistent with other climate studies in the region, suggesting a shift of precip into the cool season. Our case study reveals the importance of this shift, but we need a better understanding of who will be the winners and losers under this shift in precip. Perennial shrubs and cacti may benefit, C4 grasses and summer annuals may lose ground. We have not made quantitative forecasts but current trends are expected to continue or strengthen, depending on emissions, so we can say that the future looks bright for cholla. Mention here this is considered a pest species. 

Discuss larger-scale climate cycles like ENSO and PDO?

Add something about climate variability? This is apparent in PC 3, which I think it primarily monsoon precip. 
...
#params_summ$X97.5.[which(params_summ$X=="surv.bsize")]
@
