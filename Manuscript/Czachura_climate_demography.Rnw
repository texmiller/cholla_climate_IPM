...
Yet, it is not always apparent which dimensions of climate are most important for a given species, and even subtle environmental changes can elicit strong ecological responses (cite some tipping point papers). .
Previous studies have shown that different components of climate change may have independent effects on different aspects of demography or physiology, such as distinct effects of temperature means and extremes in alpine butterflies \citep{buckley2012demographic}.
...
Previous studies provide heterogeneous results regarding cactus responses to climate drivers, with evidence for both positive \citep{} and negative \citep{} effects of drought conditions on various aspects of cactus ecology. 
Given the important role cacti and other CAM plants in drylands worldwide \citep{}, there is a need to better understand their responses to past and future climate change. 
...
(3) Back-cast climate-dependent population growth to determine whether the past century included periods that were favorable for population growth, and (4) Identify which aspects of climate change and which demographic responses to climate most strongly determine temporal trends in population growth.
...
Seeds in the $2$-year old seed bank are assumed to either germinate (probability $\gamma_{1}$) or die. 
...
Second, we back-casted $\lambda$ over the $20^{th}$ century using time series of PC values. 
We used Life Table Response Experiments (LTREs) to decompose which combinations of climate PCs and vital rate responses were most strongly responsible for temporal fluctuations in the back-casted time series of $\lambda$.
While we are interested in temporal fluctuations in $\lambda$, we assume that these fluctuations are due entirely 
