...
``Climate'' in these models was each of the three PCs, so there were a total of $10$ candidate variables for each vital rate model, which also included random effects of year and plot or spatial block (Appendix B). 
...
Because our analyses rely on extrapolation to climate states that were observed infrequently or not at all during the demographic study, we sought to simplify the statistical models
