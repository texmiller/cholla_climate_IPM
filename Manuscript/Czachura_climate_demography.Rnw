...
\begin{figure}[h!]
  \label{fig:PCA}
<<PCA results,echo=F, message=F, warning=F,fig.width=7,fig.height=7,fig.align='center'>>=
setwd("C:/Users/tm9/Desktop/git local/cholla_climate_IPM")
...
@
  \caption{Principal component analysis (PCA) of eight seasonal climate variables for demographic transition years (May 1 -- April 30) of \Sexpr{min(PCclim$Year_t)} through \Sexpr{max(PCclim$Year_t)}. 
  ...
  PC1: $F_{\Sexpr{anova(PC1mod)["Df"][1,1]},\Sexpr{anova(PC1mod)["Df"][2,1]}} = \Sexpr{round(anova(PC1mod)["F value"][1,1],2)}$, $P < \Sexpr{pmax(0.001, round(anova(PC1mod)["Pr(>F)"][1,1],2))}$; 
  PC2: $F_{\Sexpr{anova(PC2mod)["Df"][1,1]},\Sexpr{anova(PC2mod)["Df"][2,1]}} = \Sexpr{round(anova(PC2mod)["F value"][1,1],2)}$, $P < \Sexpr{pmax(0.001, round(anova(PC2mod)["Pr(>F)"][1,1],2))}$; 
  PC3: $F_{\Sexpr{anova(PC3mod)["Df"][1,1]},\Sexpr{anova(PC3mod)["Df"][2,1]}} = \Sexpr{round(anova(PC3mod)["F value"][1,1],2)}$, $P < \Sexpr{pmax(0.001, round(anova(PC3mod)["Pr(>F)"][1,1],2))}$.}
\end{figure}
