...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/LTRE.pdf}
  \caption{LTRE decomposition of inter-annual variability in population growth rates (based on climate alone). Bars show how each vital rate, responding to each climate PC, accounted for the overall pattern of inter-annual variation in $\lambda$. Survival dominated all vital rate responses to all climate PCs. Values for flowering and fertility were non-zero but imperceptible on this scale.}
  \label{figure:LTRE}
  \end{center}
\end{figure}
