...
setwd("C:/Users/tm634/Desktop/git local/cholla_climate_IPM")
#knitr::opts_knit$set(root.dir = 'C:/Users/tm634/Desktop/git local/cholla_climate_IPM')
...
Extinction debt is a form of transient dynamics whereby populations persist despite having population growth rates that fall below replacement level ($\lambda=1$), suggesting a long-term trajectory toward local extinction but with long time lags \citep{hastings2018transient,kuussaari2009extinction}.
...
Previous studies have generated heterogeneous results regarding cactus responses to climate drivers.
...
We also sought to identify which aspects of climate are changing most strongly, and then ask whether the strongest features of climate change are the most important determinants of population responses.
...
\item{Back-cast climate-dependent population growth to determine whether the past century included periods that were favorable for population viability}
\item{Identify which aspects of climate (cool and warm season temperature and precipitation) are changing most strongly, and which demographic responses to climate (growth, survival, reproduction) most strongly determine temporal trends in population growth}
\end{itemize}
...
We built generalized linear mixed models (GLMM) in a hierarchical Bayesian framework to connect inter-annual demographic variability to climate drivers, as captured by three PCs that collectively explained \Sexpr{ms_quantities$var_explained}\% of the inter-annual variation in seasonal climate values (Results).
...
We incorporated two types of uncertainty into back-casted values of $\lambda$: imperfect knowledge of the parameter values (``estimation error'') and year-to-year fluctuations that were not related to climate (``process error''), which was estimated from the variances of random year effects. 
For the years of demographic data collection (2004--2017), we could quantify the deviations between predicted $\lambda$ based solely on climate and ``observed'' $\lambda$ that reflects climate and non-climate year effects (quotations indicate that these are the asymptotic predictions given the vital rates observed in that year). 
...
There was a positive temporal trend in $\lambda$ since 1900, and the rate of increase was \Sexpr{ms_quantities$lambda_timesgreater} times greater since 1970 compared to the overall trend since 1900 (solid vs. dashed lines in Fig. \ref{figure:lambda_year}A). 
...
For the years of direct demographic observations we were able to incorporate additional sources of variability, estimated as statistical random effects, in year-specific estimates of $\lambda$ (points in Fig. \ref{figure:lambda_year}A).
...
Thus, while there was a clear climate signal to historical trends of population growth, there was also uncertainty in these trends due to process error, i.e., heterogeneity in vital rates across years that could not be attributed to the climate PCs. 

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=\linewidth]{Figures/lambda_year_uncertainty.pdf}
  \caption{
  \textbf{A}, Posterior probability distribution for the time series of asymptotic population growth rates ($\lambda$) predicted based on inter-annual variation in climate. 
Thick black line shows the mean prediction and shaded regions show the 25, 50, 75, and 95\% credible regions accounting for both parameter uncertainty and process error. 
Vertical line separates years that were back-casted versus years that were directly observed. 
The observation years (2004 and later) include estimates for year-specific population growth rates that incorporate additional, non-designated sources of inter-annual demographic variation (points), captured statistically as year-specific random effects. 
...
Instead, temporal trends in population viability were dominated by more subtle climatic factors with relatively weak signals of recent change (the trend toward wetter and milder cool seasons, captured by PCs 2 and 3).
...
Given the potentially long lifespans of these plants, it may requires several decades more for the legacy of 20$^{th}$-century land use to fade. 
...
However, incomplete buffering of survival led to greater mortality in years with cold and dry cool-seasons -- years that are becoming less frequent under climate change (Fig. \ref{figure:PCA}) -- and these survival responses dominated the overall increase in population viability over the past 120 years (Fig. \ref{figure:lambda_year}). 
...
In our study, we detected an important role for winter minimum temperature and observed high mortality following record low winter temperatures over a multi-day deep-freeze in 2011 (this is the low outlier in Fig. \ref{figure:lambda_year}). 
...
The climate variables are the three principal components ($PC1$, $PC2$, $PC3$) of inter-annual variation in temperature and precipitation. 
...
The integral is evaluated from the lower ($L$) to upper ($U$) bounds of the plant size distribution.
...
For analysis, we discretize $x$ into $n$ bins, replacing the continuous kernel with an $n$-by-$n$ matrix (because our model also included two additional discrete states, the final projection matrix had dimensions $n+2$-by-$n+2$).
We used $n=200$ bins. 
...
We compared the observed size distribution and the predicted distribution at the long-term mean climate ($PC_{1}=PC_{2}=PC_{3}=0$).

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.75\linewidth]{Figures/SSD.pdf}
  \caption{}
  \label{figure:LTRE}
  \end{center}
\end{figure}
