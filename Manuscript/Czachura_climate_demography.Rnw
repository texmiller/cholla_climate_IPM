...
%--------------------------------------------------------------------
\section*{Discussion}
The key finding of this study is that the strongest features of climate change in this system are not the main drivers of population responses. 
Specifically, the dominant feature of climate change in our study region was the accelerating trend toward years that are warmer and drier in both cool- and warm-seasons (captured by PC1), but this was the least important contributor to the back-casted increase in $\lambda$ over the past century. 
Instead, we found that temporal trends in the viability of our focal population were dominated by more subtle climatic factors with relatively weak signals of recent change: the trend toward wetter and milder cool seasons, as captured by PCs 2 and 3. 
Our results highlight the challenges of forecasting population responses to climate change, since the strongest and therefore most detectable features of climate change may not be the most important drivers of population responses.
At the same time, our work highlights how population biologists can meet this challenge through careful accounting of various climate influences within a demographic modeling framework.

Extinction debt paragraph, with new hypotheses for why these plants have been persisting (land use?). Peters and Yao say grazing has been excluded since 1973.

Relate results to other literature on climate change-demography, especially in plants. Historically not many studies made the climate-demography connection explicitly, though this is changing, especially now that the IPM framework makes inclusion of abiotic covariates so straightforward. 
...
The climate drivers that we detected are consistent with what is known of cactus physiology and climate relations.
...
Downscaled climate data and use of 3 PCs means that we lost some resolution of climate drivers. Emphasize catostophic winter of 2011, yet it did not register very strongly in our analyses. These events are projected to become less frequent. Also, we decided to aggregate data in a particular way (seasonal). Other approaches consider more fine-grained variation in climatic factors (Teller and Ogle methods). There may also be delayed effects (Brigitte) that we have missed. 

We have no climate dependence in recruitment, and this is the emphasis of much of the cactus literature (but only because they can reconstruct cohorts through size). We can justify why even strong climate effects on recruitment would probably not affect our conclusions. 
...
\begin{table}[h!]
  ...
  \begin{center}
    \begin{tabular}{p{5cm}p{2cm}p{2cm}p{3cm}}
      ...
      \hline
      \multirow{2}{*}{Survival coefficients} & test & test & test\\
       & test & test & test\\
