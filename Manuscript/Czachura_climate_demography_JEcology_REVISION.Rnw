...
\lambda_{t} = \bar{\lambda} + \sum_{i=1}^{3} \gamma_{i}PC_{i,t}
\end{align}
...
\gamma_{i} \approx \sum_{j=1}^{n} \frac{\partial \bar{\lambda}}{\partial \theta_{j}} \frac{\partial \theta_{j}}{\partial PC_{i}} 
\end{align}
...
This may be an issue of applied concern in the region since \textit{C. imbricata} is considered undesirable \linelabel{mod12}\revise{due to its unpalatability to livestack} \citep{allen1991diffusion}. 
