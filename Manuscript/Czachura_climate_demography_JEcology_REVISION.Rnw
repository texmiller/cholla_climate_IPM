...
We further explored the implications of using downscaled data by repeating all our analyses SEV-LTER meteorological data and comparing results between the two data sources (Appendix A).
}
...
This suggests that our qualitative inference regarding the temporal trend in $\lambda$ is robust to the loss of resolution associated with downscaled climate data.}
...
\linelabel{mod6}A second limitation is that our approach to quantifying climate drivers knowingly forfeits some information, and in two ways. 
...
While these two types of data were correlated, they were not perfectly so (Appendix A); this was especially true for temperature minima and maxima (Table \ref{tab:WNASEV}), where downscaled data likely mis-estimate localized extremes. 
It is noteworthy that the downscaled climate data poorly captured the extreme deep-freeze of winter 2011  (Fig. \ref{fig:WNASEV_temp}A).
...
This allowed us to evaluate the hypothesis that recent climate change has driven \textit{C. imbricata} in our region into extinction debt, \linelabel{mod14}\revise{a hypothesis that our data fail to support}.
...
There was an extreme-cold event in 2010 that was particularly poorly captured by the downscaled data (Fig. \ref{fig:WNASEV_temp}A).
...
For simulations into historical climates more extreme than observed, this approach pins demographic responses to equal the responses at observed extrema, as can be seen in $\lambda$ responses to PC variation. 
