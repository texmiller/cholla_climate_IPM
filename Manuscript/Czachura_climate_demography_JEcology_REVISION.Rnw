...
We found that the two data sets were \revise{generally well correlated} (Table \ref{tab:WNASEV}, Fig. \ref{fig:WNASEV_temp},\ref{fig:WNASEV_precip}), which bolstered our confidence that ClimateWNA provided locally accurate climate data for both the demographic observation period as well as historical years that preceded our study. 
...
The weakest correlation was that of warm-season maximum temperature (Fig. \ref{fig:WNASEV_temp}F; Spearman's $r = \Sexpr{round(warm_max$estimate,2)}$, $P = \Sexpr{round(warm_max$p.value,2)}$). 
...
Both time series yielded a positive temporal trend, though the mean change in $\lambda$ per year was \Sexpr{ms_quantities$extrap_effect}\% weaker for the entire time series and \Sexpr{ms_quantities$extrap_effect_1970}\% weaker since 1970 when vital rates were not extrapolated (Fig. \ref{fig:lambda_PC_extrapF}).
