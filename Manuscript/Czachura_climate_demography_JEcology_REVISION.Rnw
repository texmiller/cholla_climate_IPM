...
Both time series yielded a positive temporal trend, though the mean change in $\lambda$ per year was \Sexpr{ms_quantities$extrap_effect}\% weaker for the entire time series and \Sexpr{ms_quantities$extrap_effect_1970}\% weaker since 1970 when vital rates were not extrapolated.
The minor influence of extrapolation was due to the fact that we relied most heavily on extrapolation for PC1 (Fig. \ref{fig:climate_coverage_histograms}). 
